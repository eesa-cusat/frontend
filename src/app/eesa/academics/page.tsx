...
export default function AcademicsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "schemes" | "subjects" | "resources" | "upload">("overview");
  ...
  useEffect(() => {
    ...
    if (savedTab && ["overview", "schemes", "subjects", "resources", "upload"].includes(savedTab)) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const schemesResponse = await fetch(`${API_BASE_URL}/academics/schemes/`, {
        credentials: 'include',
        headers: {
      ...
      if (schemesResponse.ok) {
        ...
        const schemesArray = Array.isArray(schemesData) ? schemesData : (schemesData.results || []);
      ...
      try {
        ...
        for (const scheme of schemes) {
          for (let semester = 1; semester <= 8; semester++) {
            try {
              const subjectsResponse = await fetch(`${API_BASE_URL}/academics/subjects/?scheme=${scheme.id}&semester=${semester}`, {
                credentials: 'include',
                headers: {
                  'Content-Type': 'application/json',
                },
              ...
              if (subjectsResponse.ok) {
                ...
                const subjectsList = (subjectsData.results || subjectsData) as SubjectListItem[];
                
                // Convert to full Subject objects
                for (const subject of subjectsList) {
                  try {
                    const detailResponse = await fetch(`${API_BASE_URL}/academics/subjects/${subject.id}/`, {
                      credentials: 'include',
                      headers: {
                        'Content-Type': 'application/json',
                      },
                    ...
                    if (detailResponse.ok) {
                      const detailedSubject = await detailResponse.json() as Subject;
      ...
      const resourcesResponse = await fetch(`${API_BASE_URL}/academics/resources/`, {
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (resourcesResponse.ok) {
        ...
        const detailedResources = await Promise.all(
          resourcesList.map(async (resource: Omit<Resource, 'is_approved'>) => {
            try {
              const detailResponse = await fetch(`${API_BASE_URL}/academics/resources/${resource.id}/`, {
                credentials: 'include',
                headers: {
                  'Content-Type': 'application/json',
                },
...
function OverviewTab({ schemes, subjects, resources }: { schemes: Scheme[], subjects: Subject[], resources: Resource[] }) {
  return (
...
function SchemesTab({ schemes, onUpdate }: { schemes: Scheme[], onUpdate: () => void }) {
  ...
  const handleUpdateScheme = async (scheme: Scheme) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/academics/schemes/${scheme.id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      toast.error(error instanceof Error ? error.message : "Failed to update scheme");
    }
  };
  ...
  const handleDeleteScheme = async (schemeId: number) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/academics/schemes/${schemeId}/`, {
        method: 'DELETE',
        headers: {
          'X-CSRFToken': csrfToken,
        },
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to create scheme");
      setIsCreating(false);
    }
  };
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setNewScheme({ ...newScheme, year: parseInt(e.target.value) })}
              />
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEditingScheme({ ...editingScheme, year: parseInt(e.target.value) })}
              />
            </div>
            <div className="flex space-x-2">
              <Button onClick={() => handleUpdateScheme(editingScheme)}>Update</Button>
...
function SubjectsTab({ schemes, onUpdate }: { schemes: Scheme[], onUpdate: () => void }) {
  ...
  const loadSubjects = async (scheme: number, semester: number) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/academics/subjects/?scheme=${scheme}&semester=${semester}`, {
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        const subjectsList = (listData.results || listData) as SubjectListItem[];
  ...
  const handleCreateSubject = async () => {
    if (!newSubject.name.trim() || !newSubject.code.trim() || !newSubject.scheme) {
    ...
    try {
      ...
      if (selectedScheme === newSubject.scheme && selectedSemester === newSubject.semester) {
  ...
  const handleUpdateSubject = async (subject: Subject) => {
    try {
      ...
      if (!subject.scheme?.id) {
        toast.error('Subject scheme information is missing. Please refresh and try again.');
        return;
      }
      ...
      const response = await fetch(`${API_BASE_URL}/academics/subjects/${subject.id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': csrfToken,
  ...
  const handleDeleteSubject = async (subjectId: number) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/academics/subjects/${subjectId}/`, {
        method: 'DELETE',
        headers: {
          'X-CSRFToken': csrfToken,
        },
    ...
    <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewSubject({ ...newSubject, scheme: Number(e.target.value) || null })}
                >
      ...
      {editingSubject && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex space-x-2">
              <Button onClick={() => handleUpdateSubject(editingSubject)}>Update</Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select scheme and semester to view subjects</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
      ...
      {isLoadingSubjects ? (
        ...
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-gray-800">Please select a scheme to view subjects</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="text-center py-8">
            <BookOpen className="h-12 w-12 mx-auto text-gray-600 mb-4" />
            <p className="text-gray-800">No subjects found for the selected scheme and semester</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4">
              ...
              <CardContent className="flex justify-between items-center p-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {subject.code} • Semester {subject.semester} • {subject.department} • {subject.credits} credits
                  </p>
                  <p className="text-xs text-gray-500">
                    Scheme: {subject.scheme?.name || 'N/A'} ({subject.scheme?.year || 'N/A'})
                  </p>
                </div>
                <div className="flex space-x-2">
                  <Button 
...
function ResourcesTab({ subjects, schemes, resources, onUpdate }: { subjects: Subject[], schemes: Scheme[], resources: Resource[], onUpdate: () => void }) {
  ...
  const filterResources = useCallback(() => {
    ...
    if (selectedApprovalStatus !== "all") {
      ...
      filtered = filtered.filter(resource => (resource.is_approved ?? false) === isApproved);
    }

    if (selectedSubject !== "all") {
      const subjectId = parseInt(selectedSubject);
      filtered = filtered.filter(resource => resource.subject.id === subjectId);
    }

    if (selectedScheme !== "all") {
      const schemeId = parseInt(selectedScheme);
      filtered = filtered.filter(resource => resource.subject.scheme.id === schemeId);
    }

    if (selectedSemester !== "all") {
      const semester = parseInt(selectedSemester);
      filtered = filtered.filter(resource => resource.subject.semester === semester);
    }
  ...
  }, [resources, selectedCategory, selectedApprovalStatus, selectedSubject, selectedScheme, selectedSemester, viewMode]);
  ...
  const handleUpdateResource = async (resource: Resource) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/academics/resources/${resource.id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': csrfToken,
  ...
  const handleApproveResource = async (resourceId: number) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/academics/toggle-approval/${resourceId}/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': csrfToken,
      ...
      const data = await response.json();
      toast.success(data.message || "Resource approval status updated successfully");
      onUpdate();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold">Manage Resources & Verify Notes</h2>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditingResource({ ...editingResource, module_number: e.target.value ? Number(e.target.value) : null })}
                >
              ...
              <div className="flex items-center space-x-2 pt-6">
                <input
                  ...
                  onChange={(e) => setEditingResource({ ...editingResource, is_approved: e.target.checked })}
                />
                <Label htmlFor="editResourceApproved">Approved</Label>
              </div>
            </div>
            <div className="flex space-x-2">
              <Button onClick={() => handleUpdateResource(editingResource)}>Update</Button>
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-indigo-50">
        <CardHeader className="pb-4">
          <CardTitle className="text-2xl font-bold text-gray-800">Filter & View Resources</CardTitle>
          <CardDescription className="text-base text-gray-600">Comprehensive filtering and verification tools</CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          {/* View Mode Selection */}
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            <Label className="text-lg font-semibold text-gray-700 mb-3 block">View Mode</Label>
            <div className="flex gap-3 flex-wrap">
              <Button
                variant={viewMode === "all" ? "default" : "outline"}
                size="lg"
                onClick={() => setViewMode("all")}
                className={`font-medium transition-all duration-200 ${viewMode === "all" ? "bg-blue-600 hover:bg-blue-700 shadow-md" : "border-gray-200 hover:border-blue-300"}`}
              >
              ...
              <Button
                ...
                size="lg"
                className={`font-medium transition-all duration-200 ${viewMode === "approved" ? "bg-green-600 hover:bg-green-700 shadow-md" : "border-green-200 hover:border-green-400 text-green-700"}`}
                onClick={() => setViewMode("approved")}
              >
                Approved ({resources.filter(r => r.is_approved === true).length})
              </Button>
              <Button
                variant={viewMode === "unapproved" ? "default" : "outline"}
                size="lg"
                className={`font-medium transition-all duration-200 ${viewMode === "unapproved" ? "bg-orange-600 hover:bg-orange-700 shadow-md" : "border-orange-200 hover:border-orange-400 text-orange-700"}`}
                onClick={() => setViewMode("unapproved")}
              >
                Pending Review ({resources.filter(r => r.is_approved !== true).length})
              </Button>
            </div>
          </div>
          ...
          {/* Advanced Filters */}
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            <Label className="text-lg font-semibold text-gray-700 mb-4 block">Advanced Filters</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="schemeFilter" className="text-sm font-medium text-gray-700">Scheme</Label>
              <select
                id="schemeFilter"
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 font-medium"
                value={selectedScheme}
              ...
              >
                ...
                {uniqueSchemes.map((scheme) => (
                  <option key={`scheme-${scheme.id}`} value={scheme.id.toString()}>
                    {scheme.name} ({scheme.year})
                  </option>
                ))}
              </select>
            ...
            <div className="space-y-2">
              <Label htmlFor="semesterFilter" className="text-sm font-medium text-gray-700">Semester</Label>
              <select
                id="semesterFilter"
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 font-medium"
                value={selectedSemester}
            ...
            <div className="space-y-2">
              <Label htmlFor="subjectFilter" className="text-sm font-medium text-gray-700">Subject</Label>
              <select
                id="subjectFilter"
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 font-medium"
              ...
              >
                ...
                {uniqueSubjects.map((subject) => (
                  <option key={`subject-${subject.id}`} value={subject.id.toString()}>
            ...
            <div className="space-y-2">
              <Label htmlFor="approvalFilter" className="text-sm font-medium text-gray-700">Status Filter</Label>
              <select
                id="approvalFilter"
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 font-medium"
          ...
          <div className="bg-gradient-to-r from-gray-50 to-blue-50 p-6 rounded-xl border border-gray-100">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 text-center">Resource Statistics</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center bg-white p-4 rounded-lg shadow-sm border">
                <div className="font-bold text-2xl text-blue-600">{filteredResources.length}</div>
                <div className="text-sm text-gray-600 font-medium">Filtered Results</div>
              </div>
              <div className="text-center bg-white p-4 rounded-lg shadow-sm border">
                <div className="font-bold text-2xl text-green-600">
                  {filteredResources.filter(r => r.is_approved === true).length}
                </div>
              ...
              <div className="text-center bg-white p-4 rounded-lg shadow-sm border">
                ...
                <div className="text-sm text-gray-600 font-medium">Total Subjects</div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      {filteredResources.length === 0 ? (
        <Card className="border-0 shadow-lg">
          <CardContent className="text-center py-16">
            ...
            <p className="text-gray-600 mb-4">No resources match your current filter selection.</p>
            {viewMode !== "all" && (
              <p className="text-sm text-blue-600 bg-blue-50 p-3 rounded-lg inline-block">
                💡 Try switching to &ldquo;All Resources&rdquo; view or adjusting your filters to see more results.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <div className="grid gap-6">
          {filteredResources.map((resource) => {
            const subject = subjects.find(s => s.id === resource.subject.id) || resource.subject;
            const scheme = schemes.find(s => s.id === subject.scheme?.id) || subject.scheme;
            return (
              <Card 
                key={resource.id} 
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5 p-5 bg-white rounded-lg border border-gray-100 shadow-sm">
                        <div>
                          ...
                          <div className="font-bold text-base text-gray-800">{subject?.code}</div>
                          <div className="text-sm text-gray-600 truncate">{subject?.name}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Scheme</div>
                          <div className="font-bold text-base text-gray-800">{scheme?.name}</div>
                          <div className="text-sm text-gray-600">{scheme?.year}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Semester</div>
                          <div className="font-bold text-base text-gray-800">Semester {subject?.semester}</div>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center gap-6 text-base text-gray-700 mb-3 font-medium">
                          ...
                          <span className="flex items-center gap-2">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-4 8a3 3 0 01-3-3V8a3 3 0 116 0v4a3 3 0 01-3 3z" />
                            </svg>
                          ...
                          {resource.like_count && (
                            <span className="flex items-center gap-2">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2v0a2 2 0 00-2 2v5m-4 0H4m4 0L4.9 14.1M4 10v10a2 2 0 002 2h2.2" />
                              </svg>
                      ...
                      <div className="flex space-x-2">
                        {resource.is_approved !== true ? (
                          ...
                          <Button 
                            ...
                            onClick={() => handleUnapproveResource(resource.id)}
                            disabled={approving === resource.id}
...
function UploadTab({ subjects, onUpdate }: { subjects: Subject[], onUpdate: () => void }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success(`${files.length} file(s) uploaded successfully (pending approval)`);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button
            ...
            disabled={isUploading || !selectedSubject || !files || !title.trim()}
            className="w-full"
          >
            {isUploading ? (
              <>Uploading...</>
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Academics Panel</h1>
                <p className="text-sm text-gray-600">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {isLoading ? (
          ...
          <>
            {activeTab === "overview" && (
              <OverviewTab schemes={schemes} subjects={subjects} resources={resources} />
            )}
            {activeTab === "schemes" && (
            ...
            {activeTab === "resources" && (
              <ResourcesTab subjects={subjects} schemes={schemes} resources={resources} onUpdate={loadData} />
            )}
            {activeTab === "upload" && (
